#!/usr/bin/env node
/**
 * Apply Michelle Morris reviewer attribution to Imago hub answers.
 *
 * Dry run (default):
 *   node scripts/review-imago-answers.mjs
 *
 * Apply to Supabase:
 *   node scripts/review-imago-answers.mjs --apply
 */
import { mkdirSync, readFileSync, writeFileSync } from 'node:fs';
import { createClient } from '@supabase/supabase-js';
import { resolveSupabaseConfig } from './lib/supabase-env.mjs';

const REVIEWER = 'michelle-morris-lpc';
const REPORT_PATH = 'reports/reviewer-attribution/michelle-morris-lpc.json';
...
async function main() {
  ...
  console.log(`\nImago reviewer attribution — ${REVIEWER}`);
  ...
  for (const slug of targetSlugs) {
    ...
    if (row.reviewed_by && row.reviewed_by !== REVIEWER && row.reviewed_by !== 'codex-seo-review' && !args.overwriteReviewer) {
  ...
  const outFile = `${OUT_DIR}/apply-imago-${new Date().toISOString().replace(/[:.]/g, '-')}.json`;
